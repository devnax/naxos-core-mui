...
const FoundItem: FC<Props> = ({data, subscirberID}) => {
  ...
  return (
    <ListItem button sx={{borderRadius: 2, px: 2, minHeight: 40}} {...props}>
      {
        icon && <ListItemAvatar sx={{minWidth: 40}}>
          {
            typeof icon === 'string' ? <Avatar sx={{width: 25, height: 25, borderRadius: 0}} src={icon} alt={icon}/> : icon
          }
        </ListItemAvatar>
      }
        <Box>
          <Typography variant='body1' color={isDark ? "grey.200" : "grey.900"}>{title}</Typography>
          <Typography variant='body2' color={isDark ? "grey.200" : "grey.900"} fontSize={13}>{description}</Typography>
        </Box>
      </ListItem>
  )
}
