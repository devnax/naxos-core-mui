...
const _ListItemView = ({ item, id, active, index }: ListItemViewProps) => {
    ...
    return (
        <>
            {item.sectionTitle && (
                <Typography mt={index > 1 ? 3.5 : 0} mb={1} variant="subtitle1" fontSize={14} fontWeight={600} sx={{ opacity: 0.7 }}>
                    {item.sectionTitle}
                </Typography>
            )}
            <ListItemButton
            ...
            >
                ...
                <ListItemText>
                    ...
                    {item.subtitle && (
                        <Typography variant="subtitle2" fontSize={13} fontWeight={500} sx={{ opacity: 0.7 }}>
                            {item.subtitle}
                        </Typography>
                    )}
                </ListItemText>
            ...
            <Scrollbar>
                <List dense>
                    {items.map((item, index: number) => (
                        <ListItemView key={item._id} active={item._id === activeItem?._id} item={item} id={id} index={index} />
                    ))}
                </List>
            </Scrollbar>
        </Box>
