    ...
    return (
        <>
            ...
            <FormControl sx={{ flexDirection: vertical ? 'row' : 'column', pl: 1 }}>
                <Item form={form} name={name} value={value} label={label} {...props} />
                {items && items.map((item) => <Item key={item.label + '_redio'} form={form} name={name} value={item.value} label={item.label} {...props} />)}
            </FormControl>
        </>
    );
}
