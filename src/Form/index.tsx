...
const HTMLForm: FC<FormProps> = ({ children, formProps, onSubmit }) => (
...
const Form: FC<FormProps> = ({ children, onSubmit, includeForm, formProps, ...boxProps }) => {
    return (
        <Loader borderRadius={2} p={3} bgcolor="background.paper" {...boxProps}>
            {includeForm !== false ? (
                <HTMLForm onSubmit={onSubmit} formProps={formProps}>
                    {children}
                </HTMLForm>
            ) : (
                children
