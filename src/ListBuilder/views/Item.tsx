...
const Item: FC<ListItemProps> = ({ title, collapse, sortable, render, onClose, onTitleEdit, onCollaps, ...StackProps }) => {
   ...
   return (
      <Stack bgcolor="background.paper" p={1} borderRadius={2} {...StackProps} mb={.5}>
         <Stack
            direction="row"
         ...
         >
            <Stack flex={1} direction="row" alignItems="center" sx={{ userSelect: "none" }}>
               {
                  sortable && <IconButton size="small" sx={{ cursor: "move" }}>
                     <MoveIcon />
                  </IconButton>
               }
               <Box pl={1} flex={1}>
                  {
                     (titleEdit && onTitleEdit) && <TextField
                        size={"small"}
                        inputProps={{
                           sx: { p: .5 }
                        }}
                  ...
                  }
                  {
                     ...
                     >
                        {title}
                        {
                           (onTitleEdit && !titleEdit) && <IconButton
